# `buggy locate` — where, when, and why a suite went red
...
```
buggy locate .                 # WHERE + WHEN + WHY, prints the ranking, writes .buggy/locate.json
buggy locate . --no-bisect     # skip WHEN (fast); --good <rev> to name a known-green revision
buggy buggy .                  # buggy, the pixel bug: grey = green, red = red, click for the root cause
```
...
line, never by how many lines they execute: a count-based file ranking once put a real guilty file
...
commit with four innocent commits on top. One test goes red. `buggy locate .` with nothing else:
...
(strong, weak), 0..15, 0 a veto. `src/buggy/laws/cause.c`, generated; ported verbatim; its own
...
`buggy locate` prints both verdicts: *root cause, by the cause law* and *if the fix is code that is
...
anything. The prompts that produced the laws: `docs/laws/CAUSE_LAW_PROMPT.md` and
`docs/laws/OMISSION_LAW_PROMPT.md`.

## buggy, the pixel bug

`buggy buggy <root>` (alias `float`) keeps `locate.json` fresh — on every source change, and every `--interval` seconds
while red — and shows it as a small always-on-top badge. The badge is standard-library Tk and imports
nothing from buggy, so it runs under whichever Python here has Tk (the venv's Homebrew build has
