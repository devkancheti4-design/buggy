...
"""buggy — the pixel bug. Standard-library Tk only — it reads <root>/.buggy/locate.json, which `buggy float`
keeps fresh, and imports nothing from buggy.
...
HOME = os.path.join(os.path.expanduser("~"), ".buggy"); os.makedirs(HOME, exist_ok=True)
...
ROOT = root()
JSON = lambda: os.path.join(root(), ".buggy", "locate.json")
BUSY = lambda: os.path.join(root(), ".buggy", "locating")
...
def pick(e):
    folder = filedialog.askdirectory(title="buggy — scan which project folder?")
    if folder:
        scan_folder(folder)
