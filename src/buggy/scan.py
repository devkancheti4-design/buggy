...
"""buggy scan — a workspace of project folders, scanned one after another, watched live.
...
that did not happen.
    ...
    buggy scan <workspace> [--port 7777] [--no-bisect] [--python P]
...
_SKIP = {".git", ".venv", "venv", "__pycache__", ".pytest_cache", ".buggy", "build", "dist", "node_modules"}
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if not self._local():
            return
        # a scan runs your test suite: a page on another site must not be able to trigger it. Browsers cannot
        # attach a custom header cross-origin without a preflight this server never answers.
...
def serve(workspace: str, port: int = 7777, python: str | None = None, bisect: bool = True) -> int:
    ...
    print(f"buggy scan — {len(STATE['projects'])} project(s) under {ws}\n  http://127.0.0.1:{port}   Ctrl-C to stop")
    try:
        srv.serve_forever()
...
PAGE = r"""<!doctype html><html><head><meta charset="utf-8"><title>buggy scan</title>
<style>
...
<h1>buggy scan</h1><span id="hint" style="color:var(--dim)">pick a project, or scan all</span><span class="st" id="st">idle</span></header>
