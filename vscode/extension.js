// buggy for VS Code / Cursor / Windsurf. Plain JavaScript, no build step.
// Everything shown comes from `buggy locate --json` and the walk it writes; nothing is animated
...
function cfg(k) { return vscode.workspace.getConfiguration("buggy").get(k); }
...
async function locate(folder, quiet) {
  ...
  if (!L) {
    setStatus("$(bug) buggy", undefined, "could not run buggy — set buggy.path");
    if (!quiet) { out.appendLine(r.se || r.so); vscode.window.showErrorMessage("buggy: could not run `" + cfg("path") + " locate` — set buggy.path (a venv's bin/buggy works)."); }
    return null;
  }
  let walk = null; try { walk = JSON.parse(fs.readFileSync(path.join(root, ".buggy", "locate.json"), "utf8")).walk; } catch (e) {}
  last = { root, L, walk };
  diags.clear();
...
async function crawl() {
  if (!last || last.L.status !== "red" || !last.walk) { vscode.window.showInformationMessage("buggy: nothing to crawl to — locate first, on a red suite."); return; }
  if (crawling) return;
...
async function scan() {
  ...
  if (!folders.length) return vscode.window.showInformationMessage("buggy: open a folder first.");
  out.show(true);
...
function activate(c) {
  ctx = c;
  out = vscode.window.createOutputChannel("buggy");
  diags = vscode.languages.createDiagnosticCollection("buggy");
  status = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 50);
  status.command = "buggy.crawl"; setStatus("$(bug) buggy", undefined, "buggy: locate the bug"); status.show();
  c.subscriptions.push(status, out, diags,
    vscode.commands.registerCommand("buggy.locate", async () => {
      const f = (vscode.workspace.workspaceFolders || [])[0]; if (!f) return vscode.window.showInformationMessage("buggy: open a folder first.");
      await locate(f, false);
    }),
    vscode.commands.registerCommand("buggy.crawl", async () => { if (!last) { const f = (vscode.workspace.workspaceFolders || [])[0]; if (f) await locate(f, false); } else await crawl(); }),
    vscode.commands.registerCommand("buggy.scan", scan));
  ...
  for (const f of (vscode.workspace.workspaceFolders || [])) {
    try {
      const j = JSON.parse(fs.readFileSync(path.join(f.uri.fsPath, ".buggy", "locate.json"), "utf8"));
